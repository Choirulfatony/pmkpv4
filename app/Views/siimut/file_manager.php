    ...
    <div class="row mb-3">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div>
                    <h5 class="mb-0">
                        <i class="bi bi-folder2 me-1"></i>File Manager
                    </h5>
                    <?php if (!empty($breadcrumbs)): ?>
                    <nav style="--bs-breadcrumb-divider: '>'" class="mt-1">
                        <ol class="breadcrumb small mb-0">
                            <li class="breadcrumb-item"><a href="<?= site_url('siimut/file-manager') ?>" class="text-decoration-none">Root</a></li>
                            <?php foreach ($breadcrumbs as $crumb): ?>
                            <li class="breadcrumb-item <?= $crumb->id === ($currentFolder->id ?? null) ? 'active' : '' ?>">
                                <?php if ($crumb->id !== ($currentFolder->id ?? null)): ?>
                                <a href="<?= site_url('siimut/file-manager?folder=' . $crumb->id) ?>" class="text-decoration-none"><?= esc($crumb->file_name) ?></a>
                                <?php else: ?>
                                <?= esc($crumb->file_name) ?>
                                <?php endif; ?>
                            </li>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="<?= site_url('siimut/file-manager' . ($currentFolder->parent_id ? '?folder=' . $currentFolder->parent_id : '')) ?>" class="btn btn-sm btn-outline-secondary" title="Kembali">
                ...
                <table id="fileTable" class="table table-hover mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="width:150px">Diupload</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                ...
                                <a href="<?= site_url('siimut/file-manager?folder=' . $item->id) ?>" class="text-decoration-none fw-medium"><?= esc($item->file_name) ?></a>
                                <?php else: ?>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-secondary text-white">
                <h6 class="modal-title"><i class="bi bi-folder-plus me-2"></i>Buat Folder Baru</h6>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h6 class="modal-title"><i class="bi bi-cloud-upload me-2"></i>Upload File</h6>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="renameModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-secondary text-white">
                <h6 class="modal-title"><i class="bi bi-pencil me-2"></i>Ubah Nama Folder</h6>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form id="formRename">
                <div class="modal-body">
...
$(document).ready(function() {
    ...
    $('#formFolder').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?= site_url('siimut/file-manager/create-folder') ?>',
            type: 'POST',
            data: $(this).serialize(),
            success: function(res) {
                if (res.status) {
    ...
    $('#formUpload').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?= site_url('siimut/file-manager/upload') ?>',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#formRequestDelete').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?= site_url('siimut/file-manager/request-delete') ?>',
            type: 'POST',
            data: $(this).serialize(),
            success: function(res) {
                if (res.status) {
    ...
    $('#formRename').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?= site_url('siimut/file-manager/rename-folder') ?>',
            type: 'POST',
            data: $(this).serialize(),
            success: function(res) {
                if (res.status) {
