    ...
    <div class="form-ikp-header">
        <div class="row align-items-center">
            ...
            <div class="col-md-4 text-end">
                ...
                <a href="<?= site_url('siimut/rekap-periode-ikp') ?>" class="btn btn-light btn-sm">
...
<script>
    ...
    function loadData() {
        ...
        xhr.open('POST', '<?= site_url('siimut/ikp/get-indicators') ?>', true);
    ...
    function renderTable(response) {
        ...
        var headerHtml = '<th class="text-center" style="width:40px;">No</th>' +
            '<th style="min-width:250px;">Jenis Kejadian</th>';
    ...
    function _openModalContinue(indicatorId, departmentId, departmentName, tanggal, indicatorName, target, units, targetUnit, restricted, approvedAction, numUnit, denUnit) {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '<?= site_url('siimut/ikp/get-indicator-detail') ?>', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
    ...
    function saveData() {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '<?= site_url('siimut/ikp/save') ?>', true);
        xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
    ...
    function deleteData() {
        ...
        Swal.fire({
            ...
            var xhr = new XMLHttpRequest();
            xhr.open('POST', '<?= site_url('siimut/ikp/delete') ?>', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
    ...
    function validateData() {
        ...
        Swal.fire({
            ...
            var xhr = new XMLHttpRequest();
            xhr.open('POST', '<?= site_url('siimut/ikp/validasi') ?>', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
    ...
    function submitRequest(action) {
        ...
        Swal.fire({
            ...
            var xhr = new XMLHttpRequest();
            xhr.open('POST', '<?= site_url('siimut/ikp/request-approval') ?>', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
