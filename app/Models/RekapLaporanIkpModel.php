...
class RekapLaporanIkpModel extends Model
{
...
protected $column_order = [
    ...
    private function _getAjaxDataRekapIkp(int $indicator, int $tahun, int $bulan)
    {
        $db = db_connect();
        ...
        $builder->where('lqi.indicator_category_id', '7');
    ...
    /**
     * Ambil data rekap IKP per bulan
     */

    public function getAjaxDataRekapIkp(int $indicator, int $tahun, int $bulan)
    {
        $builder = $this->_getAjaxDataRekapIkp($indicator, $tahun, $bulan);
     ...
     */
 public function getIndicatorIkp($post, ?int $departmentId = null)
    {
        $db = db_connect();
        ...
        $sql = "
            ...
            WHERE lqi.indicator_category_id = '7'
...
/**
     * Hitung semua rekap IKP (CACHE)
     */
    public function countAllRekapIkp($post = [])
    {
        $vtahun = isset($post['vtahun']) ? (int) $post['vtahun'] : (int) date('Y');
        $userRole = session('user_role') ?? '';
        $userDepartmentId = session('department_id') ?? 0;
        ...
        $cacheKey = 'count_all_ikp_' . $vtahun . '_' . $userRole . '_' . $userDepartmentId;
        ...
        // Query sama dengan getIndicatorIkp tapi hanya COUNT
        $query = $db->query("
            SELECT COUNT(*) as total FROM (
                SELECT DISTINCT local_quality_indicator.indicator_id
                FROM local_quality_indicator
                WHERE local_quality_indicator.indicator_category_id = '7'
    ...
    public function getDepartmentsByIndicator(int $indicatorId, int $tahun, $post = [], ?int $departmentId = null)
    {
        ...
        $query = $db->query("
            ...
            AND local_quality_indicator_group.group_type = 7
    ...
    /**
     * Ambil detail indicator by ID (CI4 version of get_detail_byid_ikp)
     */
    public function getDetailByIdIkp(int $indicatorId)
    {
        $db = db_connect();
        ...
        $query = $db->query("
            ...
            AND local_quality_indicator.indicator_category_id = '7' 
            AND local_quality_indicator.indicator_record_status IN ('A')
        ", [$indicatorId]);
     ...
     */
    public function getAllDetailData(int $indicatorId, int $tahun, ?int $departmentId = null, ?int $bulan = null)
    {
        $cache = \Config\Services::cache();
        $cacheKey = 'detail_data_ikp_' . $indicatorId . '_' . $tahun . '_dept_' . ($departmentId ?? 'all') . '_bulan_' . ($bulan ?? 'all');
        ...
        DAY(lqir.result_period) AS hari,
    ...
    ");
        ...
        $data = [];
        foreach ($results as $row) {
            $key = $row->result_department_id . '_' . $row->hari;
            $data[$key] = $row;
        }
    ...
    /**
     * Hitung rekap IKP dengan filter
     */
    public function countFilteredRekapIkp($post = [])
    {
        $vtahun = isset($post['vtahun']) ? (int) $post['vtahun'] : (int) date('Y');
        $userRole = session('user_role') ?? '';
        $userDepartmentId = session('department_id') ?? 0;
    ...
    public function clearCache()
    {
        ...
        $cache->delete('count_all_ikp_');
        $cache->delete('available_group_periods_ikp');
        return true;
    }
    ...
    {
        $cache = \Config\Services::cache();
        $cacheKey = 'available_group_periods_ikp';
    ...
    public function getMonthlyDataByIndicator(int $indicatorId, int $tahun, ?int $departmentId = null): array
    {
        ...
        $indicator = $this->getDetailByIdIkp($indicatorId);
    ...
    public function getKendalaPerbaikan(int $indicatorId, int $tahun, int $bulan, ?int $departmentId = null): array
    {
        ...
        $builder->where('indicator_category_id', '7');
    ...
    /**
     * Hitung jumlah draft (belum disetujui) per departemen untuk IKP
    ...
    /**
     * Hitung jumlah draft (belum disetujui) per bulan untuk IKP
     */
