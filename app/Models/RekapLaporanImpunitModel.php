...
class RekapLaporanImpunitModel extends Model
{
    ...
    /**
     * Ambil data rekap IMPUNIT per bulan
     */

    public function getAjaxDataRekapImpunit(int $indicator, int $tahun, int $bulan)
    {
        $builder = $this->_getAjaxDataRekapImpunit($indicator, $tahun, $bulan);
     ...
     * Berdasarkan indicator_category_id = 6
     */
    public function getAllMonthlyData(array $indicatorIds, int $tahun, ?int $departmentId = null)
    {
        ...
        $builder->select("
            ...
            ROUND(
                (SUM(lqir.result_numerator_value) / NULLIF(SUM(lqir.result_denumerator_value), 0))
                * lqi.indicator_factors
            , 2) AS total_nilai,
        ...
        $builder->join('local_quality_indicator lqi', 'lqir.result_indicator_id = lqi.indicator_id', 'inner');
...
/**
     ...
     */
    public function getIndicatorImpunit($post, ?int $departmentId = null)
    {
        $db = db_connect();
        ...
        if ($effectiveDeptId !== null) {
            $builder->where("EXISTS (
                ...
                AND lqig.group_department_id = {$effectiveDeptId}
        ...
        // Order default
        $builder->orderBy('lqi.indicator_record_status', 'ASC');
        if (isset($post['order'])) {
            $col = $this->column_order[$post['order'][0]['column']] ?? 'lqi.indicator_element';
            $dir = $post['order'][0]['dir'] ?? 'ASC';
            if ($col) {
                $builder->orderBy($col, $dir);
            }
        } else {
            $builder->orderBy('lqi.indicator_element', 'ASC');
        }

        // Ambil SEMUA indikator (tidak peduli tahun) - langsung gunakan hasil query
        $allBuilder = clone $builder;
        $allBuilder->limit(10000, 0);
        $sortedResults = $allBuilder->get()->getResult();
...
/**
     * Ambil semua indicator yang punya data di tahun tertentu
     * Dari local_quality_indicator_result dengan indicator_category_id = 6
     */
    private function getIndicatorsWithData(int $tahun): array
    {
        $db = db_connect();
        $query = $db->query("
            SELECT DISTINCT CAST(lqir.result_indicator_id AS UNSIGNED) AS result_indicator_id
    ...
    /**
     * Hitung semua rekap IMPUNIT (CACHE)
     */
    public function countAllRekapImpunit($post = [])
    {
        $vtahun = isset($post['vtahun']) ? (int) $post['vtahun'] : (int) date('Y');
        $userRole = session('user_role') ?? '';
        $userDepartmentId = session('department_id') ?? 0;
    ...
    /**
     * Ambil detail indicator by ID (CI4 version of impunit) - untuk detail di modal
     */
    public function getDetailByIdImpunit(int $indicatorId)
    {
        $db = db_connect();
    ...
    /**
     * Hitung rekap IMPUNIT dengan filter
     */
    public function countFilteredRekapImpunit($post = [])
    {
        $vtahun = isset($post['vtahun']) ? (int) $post['vtahun'] : (int) date('Y');
        $userRole = session('user_role') ?? '';
        $userDepartmentId = session('department_id') ?? 0;
    ...
    public function getMonthlyDataByIndicator(int $indicatorId, int $tahun, ?int $departmentId = null): array
    {
        ...
        $indicator = $this->getDetailByIdImpunit($indicatorId);
    ...
    public function getKendalaPerbaikan(int $indicatorId, int $tahun, int $bulan, ?int $departmentId = null): array
    {
        ...
        $builder->where('indicator_category_id', '6');
    ...
    public function getActiveDepartmentsForYear(int $tahun)
    {
        ...
        return $db->table('master_institution_department mid')
            ...
            ->where('lqi.indicator_category_id', '6')
    ...
    /**
     * Hitung jumlah draft (belum disetujui) per departemen untuk IMPUnit
    ...
    /**
     * Hitung jumlah draft (belum disetujui) per bulan untuk IMPUnit
     * Return array [bulan => total_draft]
     */
