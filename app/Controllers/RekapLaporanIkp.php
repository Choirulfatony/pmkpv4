...
use App\Models\RekapLaporanIkpModel;
...
class RekapLaporanIkp extends AppController
{
    protected $rekapModel;

    public function __construct()
    {
        $this->rekapModel = new RekapLaporanIkpModel();
    }
    ...
    public function index()
    {
        ...
        if ($indicatorId) {
            $detail = $this->rekapModel->getDetailByIdIkp((int) $indicatorId);
            return $this->render('siimut/rekap_laporan_ikp_detail', [
                'judul'       => 'Detail Rekap Ikp',
                'icon'        => '<i class="bi bi-bar-chart"></i>',
                '_content'    => view('siimut/rekap_laporan_ikp_detail', [
                    'tahun'        => $tahun,
                    'bulan'        => (int) date('m'),
        ...
        return $this->render('siimut/rekap_laporan_ikp', [
            'judul'    => 'Rekap Laporan Ikp',
            'icon'     => '<i class="bi bi-bar-chart"></i>',
            '_content' => view('siimut/rekap_laporan_ikp', [
                'tahun'        => $tahun,
    ...
    /**
     * AJAX: Ambil data rekap Ikp (untuk DataTables) - Optimized
     */
    public function getAjaxDataRekapIkp()
    {
        $post = $this->request->getPost();

        log_message('error', 'getAjaxDataRekapIkp called, post: ' . json_encode($post));

        if (!$post) {
            log_message('error', 'getAjaxDataRekapIkp - POST is empty or null');
            return $this->response->setJSON(['error' => 'Invalid request - no POST data', 'post_data' => $post]);
        }

        $indicators = $this->rekapModel->getIndicatorIkp($post, $departmentId ?? null);
        ...
        foreach ($indicators as $indicator) {
            ...
            $row[] = '<div class="py-1 text-start ps-2">
                <a href="javascript:void(0);" class="fw-semibold text-decoration-none" title="Detail Rekapan Ruangan" onclick="view_detail_ikp(' . $indicator->indicator_id . ');">'
    ...
    /**
     * AJAX: Detail Ikp per ruangan (untuk DataTables)
     */
    public function getAjaxDataRekapIkpDetail()
    {
        try {
            $post = $this->request->getPost();
            ...
            $tahun = isset($post['vtahun']) ? (int) $post['vtahun'] : (int) date('Y');
            $bulan = isset($post['bulan']) ? (int) $post['bulan'] : (int) date('m');

            log_message('error', 'DETAIL: indicatorId=' . $indicatorId . ', tahun=' . $tahun . ', bulan=' . $bulan);
            ...
            // Ambil semua ruangan untuk indicator ini (filter by department jika bukan ADMIN)
            ...
            $allDetailData = $this->rekapModel->getAllDetailData($indicatorId, $tahun, $departmentId, $bulan);
            log_message('error', 'DETAIL: allDetailData count=' . count($allDetailData));

            // Ambil info indicator
            $indicatorInfo = $this->rekapModel->getDetailByIdIkp($indicatorId);
            ...
            foreach ($departments as $dept) {
                ...
                for ($hari = 1; $hari <= $daysInMonth; $hari++) {
                    $key = $dept->department_id . '_' . $hari;
                    $item = isset($allDetailData[$key]) ? $allDetailData[$key] : null;

                    if ($item == null) {
                        $row[] = '<div class="num-val" style="display:none">0</div>';
                    } else {
                        $num = $item->num ?? 0;
                        $row[] = '<div class="num-val" style="display:none">' . esc($num) . '</div>';
                    }
                }
    ...
    /**
     * AJAX: Detail Ikp per ruangan (untuk modal detail) - Optimized
     */
    public function viewDetailIkp($indicatorId = null)
    {
        if (!$this->request->isAJAX()) {
            return $this->response->setJSON(['error' => 'Invalid request']);
        }

        $data = $this->rekapModel->getDetailByIdIkp($indicatorId);
        if ($data) {
    ...
    /**
     * Export Excel Rekap Ikp - Format PMKP
     */
    public function exportExcel()
    {
        ...
        try {
            ...
            $sheet->setTitle('IKP Rekap ' . $tahun);
            ...
            // ==================== HEADER LAPORAN ====================
            // Baris 1: Judul Utama
            $sheet->setCellValue('A1', 'Capaian Insiden Keselamatan Pasien (IKP)');
            ...
            // ==================== DATA ====================
            $indicators = $this->rekapModel->getIndicatorIkp(['vtahun' => $tahun]);
            $indicatorIds = array_column($indicators, 'indicator_id');
            ...
            // Download
            $filename = 'Capaian Insiden Keselamatan Pasien (IKP) ' . $tahun . ' ' . date('YmdHis');
    ...
    /**
     * Export Excel per Indikator Ikp - Detail per Ruangan
     */
    public function exportExcelIndicator($indicatorId)
    {
        ...
        try {
            ...
            $indicator = $this->rekapModel->getDetailByIdIkp((int) $indicatorId);
            ...
            $sheet->setCellValue('A1', 'CAPAIAN INDIKATOR KESELAMATAN PASIEN (IKP)');
            $sheet->mergeCells('A1:AB1');
            $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);
            $sheet->getStyle('A1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            ...
            // Download
            $filename = 'IKP_' . $indicatorName . '_' . $tahun . '_' . date('YmdHis');
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment;filename="' . $filename . '.xlsx"');
            header('Cache-Control: max-age=0');
    ...
    /**
     * AJAX: Daily detail semua departemen untuk satu bulan (IKP)
     */
    public function getAjaxDailyDetail()
    {
        ...
        $info = $this->rekapModel->getDetailByIdIkp($indicatorId);
