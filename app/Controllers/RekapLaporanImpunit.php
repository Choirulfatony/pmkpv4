...
class RekapLaporanImpunit extends AppController
{
    ...
    public function index()
    {
        ...
        if ($indicatorId) {
            ...
            return $this->render('siimut/rekap_laporan_impunit_detail', [
                'judul'       => 'Detail Rekap IMPUnit',
                'icon'        => '<i class="bi bi-bar-chart"></i>',
                '_content'    => view('siimut/rekap_laporan_impunit_detail', [
                    'tahun'        => $tahun,
        ...
        return $this->render('siimut/rekap_laporan_impunit', [
    ...
    /**
     * AJAX: Ambil data rekap IMPUnit (untuk DataTables) - Optimized
     */
    public function getAjaxDataRekapImpunit()
    {
        $post = $this->request->getPost();

        log_message('error', 'getAjaxDataRekapImpunit called, post: ' . json_encode($post));

        if (!$post) {
            log_message('error', 'getAjaxDataRekapImpunit - POST is empty or null');
    ...
    /**
     * AJAX: Detail IMPUnit per ruangan (untuk DataTables)
     */
    public function getAjaxDataRekapImpunitDetail()
    {
        try {
            $post = $this->request->getPost();
            ...
            $indicatorInfo = $this->rekapModel->getDetailByIdImpunit($indicatorId);
    ...
    /**
     * AJAX: Detail IMPUnit per ruangan (untuk modal detail) - Optimized
     */
    public function viewDetailImpunit($indicatorId = null)
    {
        if (!$this->request->isAJAX()) {
            return $this->response->setJSON(['error' => 'Invalid request']);
        }

        $data = $this->rekapModel->getDetailByIdImpunit($indicatorId);
        if ($data) {
    ...
    /**
     * Export Excel Rekap IMPUnit - Format PMKP
     */
    public function exportExcel()
    {
        ...
        try {
            ...
            $sheet->setTitle('IMPUnit Rekap ' . $tahun);
            ...
            // ==================== HEADER LAPORAN ====================
            // Baris 1: Judul Utama
            $sheet->setCellValue('A1', 'Capaian Indikator Mutu Prioritas Unit(IMPUnit)');
            ...
            // Download
            $filename = 'Capaian Rekap Indikator Mutu Prioritas Unit (IMPUnit) ' . $tahun . ' ' . date('YmdHis');
    ...
    /**
     * Export Excel per Indikator Impunit - Detail per Ruangan
     */
    public function exportExcelIndicator($indicatorId)
    {
        ...
        try {
            ...
            // Download
            $filename = 'IMPUnit_' . $indicatorName . '_' . $tahun . '_' . date('YmdHis');
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment;filename="' . $filename . '.xlsx"');
            header('Cache-Control: max-age=0');
    ...
    /**
     * AJAX: Daily detail semua departemen untuk satu bulan (IMPUnit)
     */
    public function getAjaxDailyDetail()
    {
        ...
        $info = $this->rekapModel->getDetailByIdImpunit($indicatorId);
